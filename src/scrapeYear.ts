...
const EN_TO_NP_DIGIT: Record<string, string> = {
  "0": "०",
  "1": "१",
  "2": "२",
  "3": "३",
...
const NP_TO_EN_DIGIT: Record<string, string> = {
  "०": "0",
  "१": "1",
  "२": "2",
  "३": "3",
...
function nepaliDigitsToNumber(value: string): number {
...
function getExpectedDaySet(dayCount: number): number[] {
  return Array.from({ length: dayCount }, (_, i) => i + 1);
}

async function scrapeSingleMonth(page: Page, bsYear: number, monthNumber: number): Promise<number> {
...
async function scrapeBsYear(bsYear: number): Promise<number[]> {
